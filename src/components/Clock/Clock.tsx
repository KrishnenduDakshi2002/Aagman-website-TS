...
export  const Clock:React.FC<{value : Date; onChange: (val : Date)=>void}> = ({value,onChange}) => {
  ...
  return (
    <div className="clock__wrapper">
      <div className="clock__display">
        <div>
          <input onClick={() => {setIsHourSelected(false)}} value={`${hour}`.padStart(2, "0")} onChange={(char)=> setHour(+char.target.value)}/>
        </div>
        <p>:</p>
        <div>
          <input onClick={()=> setIsMinuteSelected(true)} value={`${minute}`.padStart(2, "0")} onChange={(char)=> setMinute(+char.target.value)}/>
        </div>
        <div>
          <button
            onClick={() => {
              if (median == "AM") setMedian("PM");
              else setMedian("AM");
            }}
          >
            {median}
        ...
        return (
          <span style={{ transform: `translate(50%) rotate(${deg}deg)` }}>
            <button
              ...
              onClick={() => {
                setHour(i + 1);
                setTimeout(() => {setIsHourSelected(true)}, 200);
              }}
            >
